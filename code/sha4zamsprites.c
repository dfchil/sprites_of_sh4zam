...
// const uint8_t texture256[] = {
// #embed <"/rgb565_vq_tw/sh4zam256.dt">
// };
...
void render_cubes_cube() {
    ...
    if (render_mode == CUBES_CUBE_MAX) {
        ...
        pvr_sprite_cxt_txr(&cxt, PVR_LIST_OP_POLY,
                           texture32.pvrformat | PVR_TXRFMT_4BPP_PAL(16),
                           texture32.width, texture32.height, texture32.ptr,
                           PVR_FILTER_BILINEAR);
    } else {
        pvr_sprite_cxt_txr(&cxt, PVR_LIST_OP_POLY,
                           texture64.pvrformat | PVR_TXRFMT_8BPP_PAL(0),
...
int main(int argc, char* argv[]) {
    ...
    if (!pvrtex_load_blob(&texture256)) return -1;
    if (!pvrtex_load_blob(&texture64)) return -1;
    if (!pvrtex_load_palette_blob(&palette256, PVR_PAL_RGB565, 0)) return -1;
    if (!pvrtex_load_blob(&texture32)) return -1;
    if (!pvrtex_load_palette_blob(
            &{ #embed(TEXBASEPATH "/pal4/sh4zam32_w.dt.pal") }, PVR_PAL_RGB565,
            256))
        return -1;
    cube_reset_state();
    while (update_state()) {
